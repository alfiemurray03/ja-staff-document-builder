...
import { CheckCircle2, X, Users, ArrowLeft, AlertTriangle, Download, ShoppingCart } from 'lucide-react';
import { useState } from 'react';

const INCLUDED = [
  'Everything in Organisation Starter',
...
export default function OrgGrowthPage() {
  const { user } = useAuth();
  const navigate = useNavigate();
  const [loading, setLoading] = useState(false);

  async function handleCheckout() {
    if (!user) { navigate('/register?plan=org_growth'); return; }
    setLoading(true);
    try {
      const res = await fetch('/api/stripe/create-checkout-session', {
        ...
        credentials: 'include',
        body: JSON.stringify({ plan: 'org_growth' }),
      });
  ...
  return (
    <>
      <Helmet>
        <title>Organisation Growth Plan — JA Document Hub</title>
      ...
      <div className="max-w-3xl mx-auto px-4 py-12">
        ...
        <div className="flex items-start gap-4 mb-8">
          ...
          <div>
            <div className="flex items-center gap-2 mb-1">
              <h1 className="text-3xl font-extrabold text-foreground">Organisation Growth</h1>
              <Badge className="bg-purple-600 text-white">Best for Teams</Badge>
              <Badge variant="secondary">£59.99/month</Badge>
            </div>
            <p className="text-muted-foreground">For growing teams who need a shared workspace and audit trail.</p>
          </div>
        </div>
        ...
        <div className="flex items-start gap-3 bg-purple-50 border border-purple-200 rounded-xl p-4 mb-6">
          ...
          <div className="text-sm text-purple-800">
            <p className="font-semibold mb-1">Need more than 5 users?</p>
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <Button onClick={handleCheckout} disabled={loading} className="flex-1">
            {loading ? 'Starting checkout…' : 'Get Started — £59.99/month'}
          </Button>
          <Button variant="outline" asChild className="flex-1">
            <Link to="/plans/org-professional">See Org Professional →</Link>
          </Button>
        </div>
      </div>
    </>
